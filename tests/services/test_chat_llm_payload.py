...
from vcenter_event_assistant.services.chat_llm_payload import (
...
def test_prepare_chat_payload_excludes_high_cpu_mem_hosts(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("vcenter_event_assistant.services.chat_llm_payload.get_settings", lambda: s)
    ctx = _minimal_ctx(
        high_cpu_hosts=[],
        high_mem_hosts=[],
    )
