...
async def test_build_digest_context_counts_and_top_rows() -> None:
    ...
    assert len(ctx.top_notable_event_groups) >= 1
    assert ctx.top_notable_event_groups[0].event_type == "HostConnectionLostEvent"
    assert ctx.top_notable_event_groups[0].notable_score == 75
    assert ctx.top_notable_event_groups[0].occurrence_count == 1
...
@pytest.mark.asyncio
async def test_build_digest_context_groups_same_event_type() -> None:
    """同一種別が複数行でも種別集約で 1 グループになる。"""
    vid = uuid.uuid4()
    base = datetime(2026, 3, 22, 12, 0, 0, tzinfo=timezone.utc)
    window_start = base - timedelta(hours=1)
    ...
    assert len(same) == 1
    assert same[0].occurrence_count == 3
    assert same[0].notable_score == 15
