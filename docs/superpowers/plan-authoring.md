...
- **ヘッダ直下の `---` の直後**に、次のブロックを置く（順序は固定とする）。
  1. **`## Git / ブランチ方針`** 見出し
  2. [`docs/snippets/git-branch-policy-for-plans.md`](docs/snippets/git-branch-policy-for-plans.md) の内容を **そのままコピー**するか、**要約1段落＋同ファイルへのリンク**のみとする。
...
- スニペット本体: [`docs/snippets/git-branch-policy-for-plans.md`](docs/snippets/git-branch-policy-for-plans.md)
