...
- `web_search` ツール説明の対象は、リクエストの `web_search_scope` で切り替わる
  （省略時は `vmware_ecosystem` = 関連製品を含む運用一般情報）
- ツールが実際にバインドされるとき（上記 1・2 を満たすとき）だけ、システムプロンプトに
  「検索してよい例 / 検索しない例」の短い指針が付与される。トーンは
  `web_search_aggressiveness`（省略時 `balanced`）で切り替わる
- UI では **設定 → チャット** の「WEB 検索の条件」でスコープ・積極度を選び、
  ブラウザの localStorage（`vea.chat_web_search_prefs`）に永続する。入力欄の
  「WEB 検索を許可」が ON の送信でのみ API に載る
