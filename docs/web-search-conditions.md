...
- `web_search` ツール説明の対象は、障害・イベントに限らず VMware vSphere および
  関連製品（ESXi / vCenter / NSX / vSAN 等）の運用一般情報（原因・対処、KB、設定手順、
  ベストプラクティス、互換性・バージョン）です
- ツールが実際にバインドされるとき（上記 1・2 を満たすとき）だけ、システムプロンプトに
  「検索してよい例 / 検索しない例」の短い指針が付与されます。入力 JSON や会話だけで
  答えられる要約・列挙では検索せず、一般的な手順・KB 等が必要なときに使う、という
  バランス方針です
