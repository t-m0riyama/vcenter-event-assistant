...
本ファイルは、実装済み機能の**フォローアップ**や**未着手の拡張**をリポジトリ内で一覧する。Issue やプロジェクトボードへ移す場合は、ここから切り出してよい。
...
チャット以外の TODO は、**上と同様に `## トピック名` のセクションをこの下（または目次順に近い位置）へ追加**する。汎用の「メモだけ」置き場にしない（見出しが無いと後から見つけにくいため）。

（現時点では、チャット話題ガード以外のセクションは未追加。）