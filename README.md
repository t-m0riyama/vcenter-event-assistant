...
- 検索プロバイダ（現在は Tavily）を構成すると、高スコアイベントの **WEB 調査**（調査結果のキャッシュをダイジェスト・チャット応答に付記）と、チャットでの **メッセージごとにオプトインする WEB 検索** が利用できる（未構成の閉域網では自動的に無効）。
...
flowchart TB
  ...
  BE -.->|WEB 調査、チャットの WEB 検索| Search
```

## 特長

- **オープンソース**（[Apache License 2.0](LICENSE)）であり、セルフ**ホスト**での動作が可能である。
- DB は **PostgreSQL / SQLite** を `DATABASE_URL` で選択できる。
...
- **WEB 調査・WEB 検索の利用時**は検索クエリが外部の検索 API に送信される（固有のホスト名・IP アドレスはクエリに含めない設計）。検索プロバイダ未構成の環境では機能自体が無効となり、外部送信は発生しない。
