...
- （任意）期間を指定して **Markdown ダイジェスト**を生成できる。環境設定により **LLM で要約・整形**できる（運用レポートの下書き用途）。**LLM によるダイジェスト補助はベータ版**であり、挙動・出力品質・設定は予告なく変わり得る。

## 特長

- **オープンソース**（[Apache License 2.0](LICENSE)）であり、**自前ホスト**が可能である。
- DB は **PostgreSQL / SQLite** を `DATABASE_URL` で選択できる（[前提](#前提)）。
- 収集は **pyVmomi** 経由である。バックエンドは **FastAPI**、フロントは **React** のダッシュボード UI である。
- 収集間隔・データ保持日数・ダイジェストのスケジュールなどを **環境変数で調整**できる（[.env.example](.env.example) および `Settings`）。
...
設計・構成の整理は [docs/plans/2026-03-21-vcenter-event-assistant-as-built.md](docs/plans/2026-03-21-vcenter-event-assistant-as-built.md) を参照する。

フロントエンドの画面例・開発コマンドは [frontend/README.md](frontend/README.md) を参照する。

開発者向けの手順（UI スクリーンショットの再取得など）は [docs/development.md](docs/development.md) を参照する。
