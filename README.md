...
バックエンドは **FastAPI**、フロントは **React**（SPA）。永続化は **`DATABASE_URL`** で **PostgreSQL** または **SQLite** を選択する。vCenter からの収集は **pyVmomi** を用い、定期ジョブ（APScheduler）と手動収集の双方から DB に書き込む。

**内部のデータフローや図の詳細**は [docs/architecture.md](docs/architecture.md) を参照する。
